...
to mime the cleaning and scaling problem of extremly heterogenous data.
These data samples can be found in the foulders "pil", "reuters", "reviews" and "twitter".

For each text kind an own parser was implemented and timed.

For a proof-of-concept the PageRank and TextRank algorithm were called with .txt files.
...
hence yielding in a tree with a one-leveld tree, containing one node (node 0),
